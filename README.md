...
## 프로젝트 개요
스포츠 동호인 팀을 위한 회비 및 지출 관리 시스템입니다. 회비 납부 관리, 지출 내역 공개, 미납 회원 관리 등 동호회 운영에 필요한 모든 기능을 제공합니다. 데이터 처리의 7단계 프로세스를 구현합니다.
...
### 지출 관리 (신규)
- 운동시설 대여비 관리 (체육관, 테니스 코트, 풋살장, 운동장, 수영장)
- 이벤트 지출 관리 (송년회, 회식, 생일 등)
- 기타 지출 관리 (음료 구매, 단체복 구매 등)
- 카테고리별 지출 통계
- 지출 내역 공개 및 조회
...
3. **지출 등록**: 지출 관리 페이지에서 운동시설, 이벤트, 기타 지출 등록
4. **현황 확인**: 대시보드에서 전체 현황 및 수입/지출 대비 확인
5. **미납 관리**: 대시보드에서 미납 회원 목록 및 미납 기간 확인
6. **분석**: 분석 페이지에서 상세 통계 및 차트 확인
...
### 지출 테이블 (expenses) - 신규
- id: 지출 ID (자동 증가)
- category: 카테고리 (운동시설, 이벤트, 기타)
- subcategory: 서브카테고리 (체육관, 테니스 코트, 송년회, 회식 등)
- amount: 지출 금액
- description: 설명
- expense_date: 지출일
- created_by: 등록자
...
### 1. 지출 내역 공개
- 모든 팀원이 지출 내역을 확인할 수 있습니다
- 운동시설 대여비, 이벤트 비용, 기타 지출을 카테고리별로 관리
- 투명한 재정 운영 지원
